...
def save_model_distill(args, epoch, model_online, online_without_ddp, model_target, target_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    checkpoint_paths = [output_dir / ('checkpoint-%s.pth' % epoch_name)]
    for checkpoint_path in checkpoint_paths:
        to_save = {
            ...
            "online_without_ddp": online_without_ddp.state_dict(),
            "target_without_ddp": target_without_ddp.state_dict(),
            "optimizer": optimizer.state_dict(),
            "epoch": epoch,
            "scaler": loss_scaler.state_dict(),
            "args": args,
...
def auto_load_model_distill(args, model_online, online_without_ddp, model_target, target_without_ddp, optimizer, loss_scaler, model_ema=None):
    output_dir = Path(args.output_dir)
    if args.auto_resume and len(args.resume) == 0:
        import glob
        all_checkpoints = glob.glob(os.path.join(output_dir, 'checkpoint-*.pth'))
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            checkpoint = torch.load(args.resume, map_location='cpu')
        msg = model_online.load_state_dict(checkpoint['model_online'])
        print(msg)
        msg = online_without_ddp.load_state_dict(checkpoint['online_without_ddp'])
        print(msg)
        msg = model_target.load_state_dict(checkpoint["model_target"])
        print(msg)
        msg = target_without_ddp.load_state_dict(
            checkpoint["target_without_ddp"])
        print(msg)
        print("Resume checkpoint %s" % args.resume)
        if 'optimizer' in checkpoint and 'epoch' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer'])
...
def remap_checkpoint_keys_r50(ckpt):
    new_ckpt = OrderedDict()
    for k, v in ckpt.items():
        if k.startswith('encoder'):
            k = '.'.join(k.split('.')[1:]) # remove encoder in the name
