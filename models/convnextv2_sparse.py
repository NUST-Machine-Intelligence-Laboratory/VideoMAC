...
class SparseConvNeXtV2_Isotropic(nn.Module):
    """ Sparse ConvNeXtV2 Isotropic
    ...
    def __init__(self, 
        ...
        self.num_classes = num_classes
        self.patch_size = patch_size
        self.stem = nn.Conv2d(in_chans, dims, kernel_size=patch_size, stride=patch_size)
        dp_rates=[x.item() for x in torch.linspace(0, drop_path_rate, depths)] 
        self.blocks = nn.Sequential(*[Block(dim=dims, drop_path=dp_rates[i], D=D) for i in range(depths)])

        self.norm = MinkowskiLayerNorm(dims, eps=1e-6)
